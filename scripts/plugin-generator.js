...
`;
        ...
        const { overwrite } = await inquirer.prompt([
            {
                type: 'confirm',
                name: 'overwrite',
                message: chalk.yellow(`⚠️  File ${fileName} already exists. Overwrite?`),
                default: false
            }
        ]);
        if (!overwrite) {
    ...
    console.log(chalk.green(`\n✓ Plugin '${answers.name}' created successfully at:`));
    ...
    if (enable) {
        ...
        if (!enabledPlugins.includes(answers.name)) {
            ...
            console.log(chalk.green(`✓ Plugin '${answers.name}' is now enabled.`));
        }
    }
